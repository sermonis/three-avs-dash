...
export default class AutonomousController {
	...
	**/
	control ( pose, wheelAngle, velocity, dt ) {
		...
		if ( nextIndex >= pathPoses.length - 1 && progress >= 1 ) {
			...
			// Determine the desired heading at the specific point
			// on the front path by lerping between prevHeading and nextHeading
			// using progress as the weight.
			...
			// Determine if the front axle is to the left
			// or right of the front path.
	...
	** @return [ nextPointIndex, progress from ( nextPointIndex - 1 ) to nextPointIndex, { 0 - 1 } ]
	**/
	findNextIndex ( frontAxlePos ) {

		const pathPoses = this.path.poses;

		// Constrain the search to just a few points surrounding
		// the current nextIndex for performance and to avoid
		// problems with a path that crosses itself.
		...
		if ( closestIndex == pathPoses.length - 1 ) {
			...
			// The nextPoint is either (closestPoint) or (closestPoint + 1).
			// Project the frontAxlePos to both of those two line
			// segments (the segment preceding closestPoint and the segment
			// succeeding closestPoint) to determine which
			// segment it's closest to.
...
/**
** @return [ pointOnSegment, progressAlongSegment { 0 - 1 } ]
**/
